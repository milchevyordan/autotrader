...
class UpdatedExpectedDeliveryDateToClient extends Step implements StepWithComponentDataInterface, EmailStepInterface, StepWithRedFlagInterface, StepWithWeekInputInterface
{
    public const NAME = 'Updated Expected Delivery Date To Client';
    ...
    {
        $latestUpdatedExpectedDeliveryDateToClient = $this->getComponentData();
        $vehicleReadyToBeDelivered = new VehicleReadyToBeDelivered($this->getModelsWorkflow());

        return new RedFlag(
            name: self::NAME,
            description: 'If the date according to update delivery has past but vehicle is not ready to be delivered',
            isTriggered: $latestUpdatedExpectedDeliveryDateToClient &&
            Carbon::parse($latestUpdatedExpectedDeliveryDateToClient['to'][1])->isBefore(Carbon::today()) &&
            (! $vehicleReadyToBeDelivered->isCompleted() || $vehicleReadyToBeDelivered->getDateFinished()?->isBefore(Carbon::today()))
        );
    }
}
