...
class MultiSelectService
{
    ...
    /**
     * Returns collection data that suits for the Multi select library from an array.
     *
     * @param  array      $dataArray      the array containing the data
     * @param  string     $idColumnName   The column name for the select value. Default 'id'.
     * @param  string     $textColumnName The column name for the select text. Default 'name'.
     * @return Collection
     */
    public static function dataForSelectFromArray(array $dataArray, string $idColumnName = 'id', string $textColumnName = 'name'): Collection
    {
        return collect($dataArray)->pluck($idColumnName, $textColumnName);
    }
}
