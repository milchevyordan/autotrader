...
class UploadHelper
{
     ...
     * @return File
     */
    public static function uploadGeneratedFile(string $fileOutput, $originalName, string $extension, string $dir = 'public'): File
    {
        if (! str_ends_with($originalName, '-')) {
            $originalName .= '-';
        }

        $uniqueName = $originalName.now()->format('Y-m-d-H-i-s-v').".{$extension}";
        $path = "{$dir}/{$uniqueName}";
