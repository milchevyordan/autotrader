...
use App\Services\DataTable\Exports\Export;
use App\Services\DataTable\Exports\ExportMapper;
use App\Traits\Enum;
...
class DataTable
{
    ...
    public function run(int $paginate = 50, ?callable $callbackBeforePaginate = null): self
    {
        ...
        $exportType = request(null)->input('export_type');
     ...
     * @param bool $exportable indicates if column should be added in exporting
     * @return self
     */
    public function setColumn(?string $relationString = null, ?string $label = null, bool $searchable = false, bool $orderable = false, bool $exactMatch = false, bool $exportable = true): self
    {
        $column = new Column(
            label: $label,
    ...
    public function setTimestamps(): self
    {
        $this
            ...
            ->setDateColumn('updated_at', 'd.m.Y H:i');
        ...
        foreach ($this->getRelations() as $relation) {
            $builder->with(
                [$relation->relationsString => function ($query) use ($relation) {
                    ...
                    if (method_exists($query->getModel(), 'trashed') && $relation->getWithTrashed()) {
                        $query->withTrashed();
                    }
    ...
    private function makeExport(string $exportType)
    {
        $exportTypeEnum = ExportType::tryFrom($exportType);

        return ExportMapper::make($exportTypeEnum, $this->getColumns(), $this->getBuilder()->get(), $this->getDateColumns());
    }
     ...
     * @return self
     */
    private function setExport(?Export $export): self
    {
        $this->export = $export;
