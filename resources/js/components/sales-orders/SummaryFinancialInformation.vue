...
const calculatePrices = () => {
    props.form.total_vehicles_purchase_price = sumCurrencyValues(props.vehicles, v => (v as Vehicle).calculation.total_purchase_price);
    ...
    props.form.total_sales_price_exclude_vat = sumCurrencyValues(props.vehicles, v => (v as Vehicle).calculation.sales_price_net);

    props.form.total_sales_price_include_vat = sumCurrencyValues(props.vehicles, v => (v as Vehicle).calculation.sales_price_incl_vat_or_margin);

    props.form.total_sales_margin = sumCurrencyValues(props.vehicles, v => (v as Vehicle).calculation.sales_margin);

    props.form.total_registration_fees = sumCurrencyValues(props.vehicles, v => (v as Vehicle).calculation.leges_vat);

    calculateBruttoNettoPrices();
};

const calculateBruttoNettoPrices = () => {
    let totalSalesPriceExcludeVatWithItemsUnits =
        Number(
    ...
    props.form.total_sales_excl_vat_with_items = convertUnitsToCurrency(
        totalSalesPriceExcludeVatWithItemsUnits
    );
    ...
    if (props.form.is_brutto) {
        ...
        props.form.total_sales_price = convertUnitsToCurrency(
            totalSalesPriceExcludeVatWithItemsUnits +
                totalVatUnits +
                bpmUnits +
                Number(
        ...
        props.form.total_sales_price = null;
    }
};
</script>
    ...
    >
        <Accordion>
            ...
            <div class="grid lg:grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                >
                    <label for="calculation_on_sales_order">
                        {{ __("Calculation On Sales Order") }}
                    </label>
                    <RadioButtonToggle
                        :key="form.calculation_on_sales_order"
                        v-model="form.calculation_on_sales_order"
                        :name="'calculation_on_sales_order'"
                        :disabled="disabled"
                    />
                    ...
                    <label for="total_sales_excl_vat_with_items">
                        {{
                            __("Sales, items and extra`s sales price excl. VAT")
                        }}
                        *
                    </label>
                    <Input
                        v-model="form.total_sales_excl_vat_with_items"
                        :name="'total_sales_excl_vat_with_items'"
                        :disabled="disabled"
                        type="text"
                        :placeholder="
                            __('Sales, items and extra`s sales price excl. VAT')
                    ...
                    <label for="total_sales_price">
                        {{ __("Total sales order price") }} *
                    </label>
                    <Input
                        v-model="form.total_sales_price"
                        :name="'total_sales_price'"
                        :disabled="disabled"
                        type="text"
                        :placeholder="__('Total sales order price')"
                        class="mb-3.5 sm:mb-0"
                        @focus="formatPriceOnFocus(form, 'total_sales_price')"
                        @blur="formatPriceOnBlur(form, 'total_sales_price')"
                    />
                </div>
            </div>
        </Accordion>
