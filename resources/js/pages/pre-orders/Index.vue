...
const handleDelete = () => {
    ...
    deleteForm.delete(route("pre-orders.destroy", deleteForm.id as number), {
        preserveScroll: true,
    });
    closeDeleteModal();
};
...
<template>
    ...
    <AppLayout>
        ...
        <Table
            ...
            :row-click-link="
                $can('edit-pre-order') ? '/pre-orders/?id/edit' : ''
            "
            :show-trashed="true"
        ...
        >
            ...
            <template #cell(status)="{ value, item }">
                <div class="flex gap-1.5">
                    {{
                        replaceEnumUnderscores(
                            findEnumKeyByValue(PreOrderStatus, value)
                        )
                    }}
                </div>
            </template>
            ...
            <template #cell(type)="{ value, item }">
                <div class="flex gap-1.5">
                    {{
                        replaceEnumUnderscores(
                            findEnumKeyByValue(ImportEuOrWorldType, value)
                        )
                    }}
                </div>
            </template>
            ...
            <template #cell(action)="{ value, item }">
                <div class="flex gap-1.5">
                    <Link
                        ...
                        :title="__('Edit pre order')"
                        class="border border-[#E9E7E7] rounded-md p-1 active:scale-90 transition"
                        :href="route('pre-orders.edit', item.id)"
                    >
                        <IconPencilSquare classes="w-4 h-4 text-[#909090]" />
                    </Link>

                    <button
                        v-if="$can('delete-pre-order')"
                        :title="__('Delete pre order')"
                        class="border border-[#E9E7E7] rounded-md p-1 active:scale-90 transition"
                        @click="
                            showModal = true;
        ...
        <Modal :show="showModal" @close="closeDeleteModal">
            ...
            >
                {{ __("Delete Pre Order #") + deleteForm?.id }} ?
            </div>
