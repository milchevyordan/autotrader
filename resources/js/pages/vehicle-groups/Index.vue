...
import GridView from "@/components/vehicle/index/GridView.vue";
...
import { VehicleGroup, VehicleGroupForm } from "@/types";
...
const modelDefaults: VehicleGroupForm = {
    id: null!,
    name: null!,
    vehicles: null!,
    creator_id: null!,
...
const storeForm = useForm<VehicleGroupForm>(modelDefaults);
const updateForm = useForm<VehicleGroupForm>(modelDefaults);
const deleteForm = useForm<VehicleGroupForm>(modelDefaults);

const store = () => {
    validate(storeForm, rules);

    storeForm.post(route("vehicle-groups.store"), {
        preserveScroll: true,
        onSuccess: () => {
            closeCreateModal();
            storeForm.reset();
...
const update = () => {
    ...
    updateForm.put(route("vehicle-groups.update", updateForm.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeUpdateModal();
            updateForm.reset();
...
const handleDelete = () => {
    ...
    deleteForm.delete(route("vehicle-groups.destroy", deleteForm.id), {
        preserveScroll: true,
    });
    closeDeleteModal();
};
...
<template>
    <Head :title="__('Vehicle Group')" />

    <AppLayout>
        <Header :text="__('Vehicle Groups')" />

        <div
            class="bg-white border border-[#E9E7E7] rounded-lg p-4 mb-4 sm:flex items-center justify-between shadow"
        >
            <GlobalSearch prop-name="vehicleGroups" />

            <button
                v-if="$can('create-vehicle-group')"
        ...
        <Modal :show="showDeleteModal" @close="closeDeleteModal">
            ...
            >
                {{ __("Delete vehicle group") }} "{{ deleteForm.name ?? "" }}" ?
            </div>
