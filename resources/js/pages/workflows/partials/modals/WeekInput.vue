...
import { camelToTitleCase, dateForInput } from "@/utils";
...
const upsertStepForm = useForm<WorkflowFinishedStepForm>({
    ...
    finished_at: dateForInput(props.step.dateFinished),
    files: null!,
    additional_value: {
        from: props.step.componentData?.from || null!,
        to: props.step.componentData?.to || null!,
    },
...
const store = () => {
    upsertStepForm.additional_value = JSON.stringify(
        upsertStepForm.additional_value
    );
...
<template>
    <Modal :show="shown" @close="closeModal">
        ...
        <div class="col-span-2 flex gap-3 mt-2 pt-1 pb-3 px-4">
            <WeekRangePicker
                :model-value="upsertStepForm.additional_value"
